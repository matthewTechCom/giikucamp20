...
const Chat: React.FC = () => {
  ...
  // 参加中のユーザー一覧
  const [joinedUsers, setJoinedUsers] = useState<{ username: string; userIcon: string }[]>([])
  // チャットメッセージ一覧
  ...
  useEffect(() => {
    ...
    const fetchUserAndConnect = async () => {
      try {
        // 1) ユーザー情報をバックエンドから取得
        const res = await axios.get<UserResponse>('http://localhost:8080/getuserinfo')
        // res.data が UserResponse であることを TS に伝えて格納
        setUser(res.data)

        // 2) WebSocket 接続
        const wsUrl = `ws://localhost:8080/rooms/${encodeURIComponent(roomName)}/ws?password=${encodeURIComponent(
          password
        )}`
            ...
            case 'MSG': {
             ...
             case 'FILE': {
              ...
              case 'JOIN': {
                ...
                setMessages((prev) => [
                  ...prev,
                  { type: 'SYSTEM', text: text || `${username} が参加しました。` },
                ])
                break
              }
              case 'LEAVE': {
                ...
                setMessages((prev) => [
                  ...prev,
                  { type: 'SYSTEM', text: text || `${username} が退室しました。` },
                ])
                break
              }
